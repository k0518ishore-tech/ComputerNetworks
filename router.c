Here is the updated code with a print statement in `updateSpecificCost` that shows the previous cost before asking for the new value.
...
void dropEdgeRecalculateDV(int n, int graph[MAX][MAX]) {
    ...
    if (r1 == r2) {
        printf("\nCannot drop edge from a router to itself.\n");
        return;
    }
    ...
    printf("\nEdge %c-%c dropped (cost set to %d).\n", r1Char, r2Char, INF);
    ...
    printf("\n========================================\n");
    printf("   DISTANCE VECTOR AFTER EDGE DROP\n");
    printf("========================================\n");
...
int main() {
    ...
    do {
        ...
        printf("8. Drop Edge & Recalculate Distance Vector\n");
        printf("9. Exit\n");
        printf("\nEnter your choice: ");
        scanf("%d", &choice);
        switch (choice) {
            case 1:
                ...
                printf("\nEnter the cost matrix. Enter %d for no connection.\n\n", INF);
                for (i = 0; i < n; i++) {
                    for (j = 0; j < n; j++) {
